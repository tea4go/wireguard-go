...
12. 接口启动完成后进入事件循环；所有接口失败时记录告警并保持进程运行，等待终止信号。
...
如果没有接口成功启动，Device 和 UAPI 不会产生事件，主循环仅等待 `SIGINT/SIGTERM`。此时必须记录与 Windows 行为一致的告警，明确进程仍保持运行，不能以接口启动失败码直接退出。
...
- 配置源无法读取或没有有效配置：记录错误并以 `ExitSetupFailed` 退出。
- 全部接口启动失败：逐个记录启动错误，再记录“当前没有成功启动的接口，进程将保持运行并等待终止信号”的告警，进入仅等待终止信号的事件循环。
...
- 同步模式优先于 VPN 启动流程。
- 零接口成功时选择保持进程运行，并使用告警级别输出等待终止信号的说明。
- 至少一个接口成功时保持现有事件循环和关闭逻辑。
